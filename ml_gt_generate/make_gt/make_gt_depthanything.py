...
--------
  ...
  5. Export left image, aligned GT, and merged RealSense depth.
...
def step3_fit_metric_L(da_L, rs_depth_L, anchor_valid, da_metric=False,
                       max_depth=30.0, fit_samples=10000,
    ...
    info = {"mode": "affine", "s": s, "t": t, "inl": inl,
            "inlier_count": inlier_count,
            "fit_sample_count": fit_sample_count, **common_info,
...
def build_montage(left_img, da_L, depth_I, rs_depth_L, anchor_valid, depth_L, info,
    ...
    gt_txt = (f"4 GT val<5m={val_mae_txt} rel={val_rel_txt} "
              f"QC={info.get('qc_status', 'n/a')}")
...
FIT_STATS_FIELDS = [
    "frame", "source_frame", "stamp", "scale", "shift",
...
def main():
    ...
    for source_frame, (t_anchor, disp_msg) in enumerate(data[args.disp_topic]):
        ...
        p10, p50, p90 = depth_percentiles(depth_L, gt_valid)
        append_stats_csv(stats_csv_path, {
            ...
            "merged_coverage": float(np.isfinite(rs_depth_L).mean()),
            "GT_valid_ratio": float(gt_valid.mean()),
